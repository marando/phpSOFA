...
trait iauEect00 {
  ...
  public static function iauEect00($date1, $date2) {
    ...
    //$NE0 = (int) (sizeof e0 / sizeof (TERM));
    //$NE1 = (int) (sizeof e1 / sizeof (TERM));
    $NE0 = (int)count($e0);
    $NE1 = (int)count($e1);
    ...
    for ($i = $NE0 - 1; $i >= 0; $i--) {
      $a = 0.0;
      for ($j = 0; $j < 8; $j++) {
        $a += (double)($e0[$i][0][$j]) * $fa[$j];
      }
      $s0 += $e0[$i][1] * sin($a) + $e0[$i][2] * cos($a);
    }

    for ($i = $NE1 - 1; $i >= 0; $i--) {
      $a = 0.0;
      for ($j = 0; $j < 8; $j++) {
        $a += (double)($e1[$i][0][$j]) * $fa[$j];
      }
      $s1 += $e1[$i][1] * sin($a) + $e1[$i][2] * cos($a);
    }
